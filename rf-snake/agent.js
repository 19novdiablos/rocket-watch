...
// ----------------------------------------------
function me_getRandomInteger(min, max) {
  return Math.floor((max - min) * Math.random()) + min
}
function me_getRandomAction() {
  return ['UP', 'DOWN', 'LEFT', 'RIGHT'][me_getRandomInteger(0, 3)]
}
export class me_SnakeGameAgent {
  constructor(game, config) {
    assertPositiveInteger(config.epsilonDecayFrames);
    this.game = game;
    this.epsilonInit = config.epsilonInit
